...
const api = {
  ...
  verbose(content) {
    this._unit.verbose(content);
  },

  enable() {
    this._unit.config.enabled = true;
  },

  disable() {
    this._unit.config.enabled = false;
  },

  interval(ms) {
    this._unit.interval(ms);
    return this;
  },

  frames(frames) {
    const logger = this._create(this._unit);
    logger._unit.write = this._unit.update;
    logger._unit.content = cine(frames);
    return logger;
  },
  ...
  animate(frames, interval) {
    ...
    const animation = cine(frames, interval);
